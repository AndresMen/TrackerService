...
import com.example.mendez.trackerservice.adapter.CustomArrayAdapter;
import com.example.mendez.trackerservice.R;
import com.example.mendez.trackerservice.model.header;
import com.example.mendez.trackerservice.model.obj_sms;
...
public class message extends Fragment implements View.OnClickListener, AdapterView.OnItemLongClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (tus.equals("error")|nom.equals("error")){
            llme.setVisibility(View.GONE);
           //Toast.makeText(getContext(), tus+" -- "+nom, Toast.LENGTH_SHORT).show();
        }else{
            //Toast.makeText(getContext(), tus+" -- "+nom, Toast.LENGTH_SHORT).show();
        ...
        if (fila.getCount()>0){
            ...
            if (fila.moveToFirst()) {
                do {
                    ...
                    content = new obj_sms(fila.getString(0),fila.getString(1),fila.getString(2),fila.getString(3),fila.getString(4));
                    list.add(content);
        ...
        switch (v.getId()){
            case R.id.fabmensa:
                ...
                if (!etem.getText().toString().trim().isEmpty()){
                    ...
                    DatabaseReference reference= FirebaseDatabase.getInstance().getReference().child("autos").child(user.getUid()).child("viaje").child("smsau").push();
                    mapasms.put("fecha",fecha);
                    mapasms.put("hora",ho);
                    mapasms.put("sms",etem.getText().toString().trim());
                ...
                public void onClick(DialogInterface dialogInterface, int i) {
                    ...
                    switch (i){
                        case 0:
                            if (item instanceof obj_sms)
                            {
                                infmes(((obj_sms) item).getMfecha(),((obj_sms) item).getMhora(),((obj_sms) item).getMnombreCli());
                            }
                            break;
                        case 1:
                            if (item instanceof obj_sms){
